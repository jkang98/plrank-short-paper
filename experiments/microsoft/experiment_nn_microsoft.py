...
def init_model():
    nn_model = nn.Sequential(
        nn.Linear(136, 107, dtype=torch.float64),
        nn.Sigmoid(),
        nn.Linear(107, 43, dtype=torch.float64),
        nn.Sigmoid(),
        nn.Linear(43, 29, dtype=torch.float64),
        nn.Sigmoid(),
        nn.Linear(29, 1, dtype=torch.float64),
    )
    return nn_model
...
optimizer = torch.optim.SGD(model.parameters(), lr=0.00760285856015538)
